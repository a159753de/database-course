...
def daily_crawler():
    ...
    stock_list = [
        # TODO : 練習3
        {"stock_code": "2330", "stock_name": "台積電", "stock_type": "tse"},
        {"stock_code": "2454", "stock_name": "聯發科", "stock_type": "tse"},
    ]
    ...
    for stock in stock_list:
        ...
        try:
            ...
            conn.commit()
            print(f"💾 {datetime.now().strftime('%H:%M:%S')} - 已寫入: {stock_name} ({parsed_data['latest_price']})")
        except Exception as e:
            print(f"❌ 寫入 {stock_code} 失敗: {e}")
    conn.close()
